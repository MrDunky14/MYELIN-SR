...
def build_val_loader(
    ...
    If subfolders like 'image_SRF_2' exist (common in Set14), uses that.
    """
    val_path = Path(data_root) / dataset_name
    
    # Check for scale-specific subfolder (common in benchmark sets)
    srf_path = val_path / f"image_SRF_{scale}"
    if srf_path.exists():
        val_path = srf_path
